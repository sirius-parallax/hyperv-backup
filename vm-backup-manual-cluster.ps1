...
if ($vms.Count -gt 0) {
    ...
    Write-Host "Enter the number(s) of VMs to backup, separated by commas (e.g., 1,3,5), or 'all' to backup all VMs"
    $selection = Read-Host
    ...
    foreach ($vm in $selectedVMs) {
        ...
        Write-Log "Preparing export for VM '$vmName' on node '$ownerNode' to $currentExportPath..."
        ...
        # Export VM
        try {
            Write-Log "Exporting VM '$vmName' to $currentExportPath on node '$ownerNode'..."
    ...
    Write-Log "No virtual machines found in the cluster."
}
